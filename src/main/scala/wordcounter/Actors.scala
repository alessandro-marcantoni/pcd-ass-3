...
object Actors {
  ...
  object Discoverer {
    def apply(stripper: ActorRef[Message]): Behavior[Message] =
  ...
  object Stripper {
    def apply(counter: ActorRef[Message]): Behavior[Message] =
  ...
  object Counter {
    def apply(
        ignoredWordsPath: String,
        nWords: Int,
        renderer: ActorRef[Message]
    ): Behavior[Message] =
      Behaviors.setup(_ => {
        val ignoredWords: List[String] = Files
          .readAllLines(Path.of(ignoredWordsPath), Charset.defaultCharset())
          .asScala
          .toList
        var occurrences: Map[String, Int] = Map()
        Behaviors.receiveMessage {
          case WordList(l) =>
            val (alreadyThere, notYet) = l
              .filter(!ignoredWords.contains(_))
              .partition(occurrences contains _)
            alreadyThere foreach (w =>
              occurrences = occurrences + (w -> (occurrences(w) + 1))
            )
            notYet foreach (w => occurrences = occurrences + (w -> 1))
            renderer ! Occurrences(
              occurrences.toList.sortWith((a, b) => a._2 > b._2).take(nWords)
            )
            Behaviors.same
        }
      })
  ...
  object Main {
    ...
    def mainBehavior(
      ...
      val counter: ActorRef[Message] = ctx.spawn(
        Counter(parameters.ignoredWordsPath, parameters.nWords, renderer),
        "counter"
      )
      val stripper: ActorRef[Message] = ctx.spawn(Stripper(counter), "stripper")
      val discoverer: ActorRef[Message] =
        ctx.spawn(Discoverer(stripper), "discoverer")
