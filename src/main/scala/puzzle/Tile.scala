...
case class Tile(image: Image,
                originalPosition: Int,
                var currentPosition: Int) extends Comparable[Tile] {
...
case class TileButton(tile: Tile) extends JButton(new ImageIcon(tile.image)) {
  addMouseListener(new MouseAdapter {
    override def mouseClicked(e: MouseEvent): Unit = setBorder(BorderFactory.createLineBorder(Color.red))
  })
}
